...
1. 始终像 Grok AI 一样使用多轮搜索，尽量获取全面详尽的信息。
...
8. **需要抓取 GitHub raw 内容时**，`webfetch` 可能超时，改用 `exa_web_fetch_exa` 或 `curl`。
